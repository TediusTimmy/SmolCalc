...
Basically, I wanted to get rid of the slow that was WTFITS while also being able to swap out number systems.
...
## More notes on differences
I changed the cell evaluation substructure. Firstly, I removed automatic reference following: now the order of evaluation is more important, because the spreadsheet doesn't try to hide it from you. I found this out with one sheet I wrote which needs a left-to-right, top-to-bottom evaluation order, rather than the default top-to-bottom, left-to-right order. NaNs abounded. Secondly, I changed the cell parser. The new parser now doesn't try to rewrite cells : cell rewriting happens at a different step by different code. This means that it is safe to save and exit at any time: no cell can be corrupted and there isn't even a race condition. Also, it means that you can now copy and paste cells that contain nonsense. However, it means that deleting rows and columns doesn't magically update cell references (copy-and-paste has been made to work). Additionally, the new parser doesn't allow some functions to be written in Backwards, because it evaluates the arguments. That said, the new parser should accept all of the same valid cells of the old parser (so long as you weren't using the old parser's lazy evaluation).


## Starting the Program

* The very first argument is one of `-0`, `-1`, `-2`, `-3`, `-4`, `-5`, `-6`, or `-7`. This is the number system to use.
...
* The following accepted argument is `-b`, which initiates batch mode. For each `-b` argument, the next argument is expected to be a formula to evaluate. The program will evaluate each batch command and then stop before entering interactive mode. This can be used to: use SmolCalc as a command-line calculator; query the contents of a spreadsheet from a shell script; or output the value of a cell whose contents are too large to see in interactive mode.
