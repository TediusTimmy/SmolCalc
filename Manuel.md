...
I changed the cell evaluation substructure. Firstly, I removed automatic reference following: now the order of evaluation is more important, because the spreadsheet doesn't try to hide it from you. I found this out with one sheet I wrote which needs a left-to-right, top-to-bottom evaluation order, rather than the default top-to-bottom, left-to-right order. NaNs abounded. Secondly, I changed the cell parser. The new parser now doesn't try to rewrite cells. This means that it is safe to save and exit at any time: no cell can be corrupted and there isn't even a race condition. Also, it means that you can now copy and paste cells that contain nonsense. However, it means that copy-and-paste doesn't magically update cell references. I found that out the hard way. Additionally, the new parser doesn't allow some functions to be written in Backwards, because it evaluates the arguments. That said, the new parser should accept all of the same valid cells of the old parser (so long as you weren't using the old parser's lazy evaluation).
